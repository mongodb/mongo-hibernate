/*
 * Copyright 2025-present MongoDB, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.mongodb.hibernate.query.select.temporal;

import jakarta.persistence.Entity;
import jakarta.persistence.Id;
import jakarta.persistence.Table;
import org.hibernate.testing.orm.junit.DomainModel;
import org.junit.jupiter.params.provider.Arguments;

import java.time.LocalTime;
import java.util.Arrays;
import java.util.List;
import java.util.TimeZone;
import java.util.stream.Stream;

@DomainModel(annotatedClasses = {SelectionLocalTimeIntegrationTest.Item.class})
public class SelectionLocalTimeIntegrationTest
        extends AbstractSelectionTemporalIntegrationTest<SelectionLocalTimeIntegrationTest.Item, LocalTime> {

    public static final List<Item> ITEMS = List.of(
            new Item(1, LocalTime.of(10, 0)), new Item(2, LocalTime.of(14, 30)), new Item(3, LocalTime.of(23, 59)));

    private static List<Item> getTestingItems(int... ids) {
        return Arrays.stream(ids)
                .mapToObj(id -> ITEMS.stream()
                        .filter(c -> c.id == id)
                        .findFirst()
                        .orElseThrow(() -> new IllegalArgumentException("id does not exist: " + id)))
                .toList();
    }

    @Override
    public List<Item> getData() {
        return ITEMS;
    }

    public static Stream<Arguments> testComparisonByEq() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(1),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class),
                // timezone has no effect on LocalTime comparison
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+12"),
                        getTestingItems(1),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testComparisonByNe() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class),
                // timezone has no effect on LocalTime comparison
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testComparisonByLt() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(23, 00),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(1, 2),
                        """
                        {"$date": "1970-01-01T23:00:00Z"}""",
                        Item.class),
                Arguments.of(
                        // timezone has no effect on LocalTime comparison
                        LocalTime.of(23, 00),
                        TimeZone.getTimeZone("GMT+12"),
                        getTestingItems(1, 2),
                        """
                        {"$date": "1970-01-01T23:00:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testComparisonByLte() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(23, 59),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(1, 2, 3),
                        """
                        {"$date": "1970-01-01T23:59:00Z"}""",
                        Item.class),
                // timezone has no effect on LocalTime comparison
                Arguments.of(
                        LocalTime.of(23, 59),
                        TimeZone.getTimeZone("GMT+12"),
                        getTestingItems(1, 2, 3),
                        """
                        {"$date": "1970-01-01T23:59:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testComparisonByGt() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class),
                // timezone has no effect on LocalTime comparison
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+12"),
                        getTestingItems(2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testComparisonByGte() {
        return Stream.of(
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+10"),
                        getTestingItems(1, 2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class),
                // timezone has no effect on LocalTime comparison
                Arguments.of(
                        LocalTime.of(10, 0),
                        TimeZone.getTimeZone("GMT+12"),
                        getTestingItems(1, 2, 3),
                        """
                        {"$date": "1970-01-01T10:00:00Z"}""",
                        Item.class));
    }

    public static Stream<Arguments> testOrderByAsc() {
        return Stream.of(
                Arguments.of(TimeZone.getTimeZone("GMT+10"), getTestingItems(1, 2, 3), Item.class),
                Arguments.of(TimeZone.getTimeZone("GMT+12"), getTestingItems(1, 2, 3), Item.class));
    }

    public static Stream<Arguments> testOrderByDesc() {
        return Stream.of(
                Arguments.of(TimeZone.getTimeZone("GMT+10"), getTestingItems(3, 2, 1), Item.class),
                Arguments.of(TimeZone.getTimeZone("GMT+12"), getTestingItems(3, 2, 1), Item.class));
    }

    @Entity(name = "Item")
    @Table(name = COLLECTION_NAME)
    static class Item {
        @Id
        public int id;

        public LocalTime temporal;

        public Item() {}

        public Item(int id, LocalTime temporal) {
            this.id = id;
            this.temporal = temporal;
        }

        @Override
        public String toString() {
            return "Item{" + "id=" + id + ", temporal=" + temporal + '}';
        }
    }
}
