...
abstract class AbstractMqlTranslator<T extends JdbcOperation> implements SqlAstTranslator<T> {
    ...
    public void visitStandardTableInsert(TableInsertStandard tableInsert) {
        ...
        astVisitorValueHolder.yield(
                ...
                new AstInsertCommand(
                        tableInsert.getMutatingTable().getTableName(), singletonList(new AstDocument(astElements))));
    }
    ...
    public void visitDeleteStatement(DeleteStatement deleteStatement) {
        ...
        var collection = getCollection(deleteStatement);
        var filter = acceptAndYield(deleteStatement.getRestriction(), FILTER);
        astVisitorValueHolder.yield(COLLECTION_MUTATION, new AstDeleteCommand(collection, filter));
    }

    @Override
    public void visitUpdateStatement(UpdateStatement updateStatement) {
        checkMutationStatement(updateStatement);
        var collection = getCollection(updateStatement);
        ...
        astVisitorValueHolder.yield(COLLECTION_MUTATION, new AstUpdateCommand(collection, filter, fieldUpdates));
    }
    ...
    public void visitInsertStatement(InsertSelectStatement insertStatement) {
        ...
        var collection = getCollection(insertStatement);
        ...
        var insertDocuments = new ArrayList<AstDocument>(valuesList.size());
        for (Values values : valuesList) {
            ...
            insertDocuments.add(new AstDocument(astElements));
        }

        astVisitorValueHolder.yield(COLLECTION_MUTATION, new AstInsertCommand(collection, insertDocuments));
    }

    private String getCollection(MutationStatement mutationStatement) {
