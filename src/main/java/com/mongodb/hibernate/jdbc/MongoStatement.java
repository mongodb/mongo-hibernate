...
final class MongoStatement extends StatementAdapter {
    ...
    public MongoStatement(
            @Nullable MongoDatabase mongoDatabase, ClientSession clientSession, MongoConnection mongoConnection) {
    ...
    @Override
    public int executeUpdate(@Nullable String mql) throws SQLException {
        checkClosed();
        if (mql == null) {
            throw new SQLException("mql is null");
        }
        ...
        try {
            ...
            if (result.getDouble("ok") != 1.0) {
                throw new SQLException(format("Failed to execute mql [%s]", mql));
            }
            return result.getInteger("n");
        } catch (RuntimeException e) {
            throw new SQLException(format("Failed to execute mql [%s]", mql), e);
        }
    }
